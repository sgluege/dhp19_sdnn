...
def plot_net_output(output, sample_idx, time_idx, joint_idx, batch_idx=None, cam_index=None):
    """
    Plot model output
    """
    plt.figure()
    ...
    if batch_idx is None:
        ...
        plt_title = f'Output sample in batch {batch_idx}\n {sample_idx}, time {time_idx}, target joint {joint_idx}'
    plt.imshow(output[sample_idx, joint_idx,:,:,time_idx])
    plt.title(plt_title)
    plt.show()


def plot_net_input_output(output, input, sample_idx, time_idx, joint_idx, batch_idx=None, cam_index=None):
    """
    Plot model output
    """
    # arget_sample = target[batch_idx, joint_idx, :, :, cam_index, time_idx]
    if batch_idx is None:
        ...
        plt_title = f'Input/Output sample in batch {batch_idx}\n {sample_idx}, time {time_idx}, target joint {joint_idx}'
    plt.figure()
...
num_stpes = 20
for batch_idx, (input, target, session, subject, mov)in itertools.islice(enumerate(train_loader), num_stpes):
    # get mini batch
    ...
    time_idx=0
    joint_idx=8
    plot_net_output(output.detach().cpu(), sample_idx=sample_idx, time_idx=time_idx, joint_idx=joint_idx, batch_idx=batch_idx)   
    plot_net_input_output(output.detach().cpu(), input.detach().cpu(), sample_idx=sample_idx, time_idx=time_idx, joint_idx=joint_idx, batch_idx=batch_idx)

    # gradient flow
    grad = model.grad_flow()
    plt.figure()
    plt.semilogy(grad)
    plt.title(f'Gradient Flow batch {batch_idx}')
    ...
    plt.title(f'Number of events per layer batch {batch_idx}')
